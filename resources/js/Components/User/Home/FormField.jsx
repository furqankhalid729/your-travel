...
const FormField = ({ icon: Icon, label, type, description, isLast, showCenterIcon, name,carSubTab }) => {
  ...
  return (
    <div className={`relative flex text-gray-700 flex-grow space-x-2 mb-4 md:mb-0 w-full ${carSubTab == "byHours" & name == "to" ? 'hidden' : ''} `}>
      <Icon className="text-red-600 text-[27px] md:text-2xl mt-1 md:mt-0 ml-2 md:ml-0" />
      <div className="flex flex-col place-items-start flex-grow">
        ...
        {(name === "from" || name === "to") && showDropdown && suggestions.length > 0 && (
          <ul className="absolute left-0 w-full bg-white border border-gray-300 mt-1 rounded-md shadow-lg z-10 top-[60px]">
